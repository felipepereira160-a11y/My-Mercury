...
def carregar_dataframe(arquivo, separador_padrao=','):
    if arquivo.name.endswith('.xlsx'):
        ...
        try:
            arquivo.seek(0); df = pd.read_csv(arquivo, encoding='latin-1', sep=separador_padrao, on_bad_lines='skip')
            if len(df.columns) > 1: return df
        except Exception: pass
        arquivo.seek(0); outro_separador = ',' if separador_padrao == ';' else ';'; df = pd.read_csv(arquivo, encoding='latin-1', sep=outro_separador, on_bad_lines='skip')
        return df
    return None
...
if st.session_state.df_dados is not None:
    ...
    # (O código do dashboard que já funciona vai aqui)
    pass
...
if st.session_state.df_mapeamento is not None:
    ...
    # (O código da consulta interativa e do mapa que já funciona vai aqui)
    pass

# --- OTIMIZADOR DE PROXIMIDADE (VERSÃO CORRIGIDA) ---
if st.session_state.df_dados is not None and st.session_state.df_mapeamento is not None:
    ...
    with st.expander("🚚 Abrir Otimizador de Proximidade de RT"):
        try:
            ...
            # Nomes de coluna esperados (hard-coded para confiabilidade)
...
if prompt := st.chat_input("Faça uma pergunta específica..."):
    # (Lógica do chat que já funciona)
    pass
