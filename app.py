...
if st.session_state.df_dados is not None:
    ...
    rep_col_dados = next((col for col in df.columns if 'representante técnico' in col.lower() and 'id' not in col.lower()), None)
    city_col_dados = next((col for col in df.columns if 'cidade agendamento' in col.lower()), None)
    motivo_fechamento_col = next((col for col in df.columns if 'tipo de fechamento' in col.lower()), None)
    ...
    with col_graf1:
        ...
        if status_col: st.bar_chart(df[status_col].value_counts())
        else: st.warning("Coluna 'Status' não encontrada.")

        st.subheader("Top 10 Cidades com Mais Ordens")
        if city_col_dados: st.bar_chart(df[city_col_dados].value_counts().nlargest(10))
        else: st.warning("Coluna 'Cidade Agendamento' não encontrada.")

    with col_graf2:
        st.subheader("Contagem por Tipo de Fechamento")
        if motivo_fechamento_col: st.bar_chart(df[motivo_fechamento_col].dropna().value_counts())
        else: st.warning("Coluna 'Tipo de Fechamento' não encontrada.")
    
        st.subheader("Top 10 Representantes com Mais Ordens")
        if rep_col_dados: st.bar_chart(df[rep_col_dados].value_counts().nlargest(10))
        else: st.warning("Coluna 'Representante Técnico' não encontrada.")
    ...
    with b_col1:
        if st.button("Top RTs com Ordens Realizadas"):
            st.write("Resultado da Análise:")
            if status_col in df.columns and rep_col_dados in df.columns:
                realizadas_df = df[df[status_col] == 'Realizada']
    ...
    with b_col2:
        if st.button("Top RTs com Visitas Improdutivas (Indisponibilidades)"):
            st.write("Resultado da Análise:")
            if motivo_fechamento_col in df.columns and rep_col_dados in df.columns:
