                ...
                if df_otimizacao_filtrado.empty:
                    ...
                    if ordens_na_cidade is not None and not ordens_na_cidade.empty:
                        ...
                        if cidade_info.empty:
                            ...
                            # Exibir cada ordem em container expandível
                            for index, ordem in ordens_na_cidade.iterrows():
                                rt_atual = ordem[os_rep_col]
                                with st.expander(f"OS: {ordem[os_id_col]} | Cliente: {ordem[os_cliente_col]}", expanded=False):
                                    col1, col2 = st.columns(2)
                                    with col1:
...
# ------------------------------------------------------------
# SEÇÃO DO CHAT DE IA (SUBSTITUÍ a implementação original do segundo por
# uma unificação do primeiro — mantendo o modelo gemini-2.5-flash)
# ------------------------------------------------------------
st.markdown("---")
st.header("💬 Converse com a IA")

# Exibe histórico do chat (mantendo display_history para compatibilidade)
for message in st.session_state.display_history:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

# Mostra também (se houver) o chat_history (compatibilidade com o primeiro)
# (não duplica mensagens — apenas mostra se houver algo extra)
...
if prompt := st.chat_input("Envie uma pergunta ou mensagem..."):
    # salva em ambos históricos para manter compatibilidade dos códigos
    ...
    # Detecta se é pergunta sobre dados ou chat genérico
    tipo = detectar_tipo_pergunta(prompt)
    resposta_final = ""

    with st.chat_message("assistant"):
        with st.spinner("Pensando..."):
            # Se for relacionado a dados e existir dataframe, chama a análise robusta (executar_analise_pandas)
            if tipo == "dados":
                # Prioriza df_dados, senão df_mapeamento
                df = st.session_state.df_dados or st.session_state.df_mapeamento
                if df is not None:
                    # usamos a função robusta que retorna (resultado, erro)
                    # hash do df para caching (como no segundo)
                    current_df = df
                    df_hash = pd.util.hash_pandas_object(current_df).sum()
                    # decidir df_type — se mapeamento ou dados
                    ...
                    if erro == "PERGUNTA_INVALIDA":
                        ...
                        if isinstance(resultado_analise, (pd.Series, pd.DataFrame)):
                            # Mostra a tabela e responde que a info está acima
                ...
                # Chat genérico: usa Gemini-2.5-flash (generate_content)
                try:
